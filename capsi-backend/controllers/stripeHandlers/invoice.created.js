...
module.exports = async function (data) {
  ...
  if (data.object.status === "draft") {
    try {
      await stripe.invoices.pay(data.object.id);
    } catch (error) {
      console.error("error in invoice payment for the draft invoice");
      console.error(error);
    }
  }
  ...
  if (invoiceDetails.metadata) {
    ...
    if (activeSubscriptionDetails) {
      invoiceDetails.subscription = activeSubscriptionDetails.id;
  ...
  await paymentModel.saveTransactionDetails(invoiceDetails);
};
